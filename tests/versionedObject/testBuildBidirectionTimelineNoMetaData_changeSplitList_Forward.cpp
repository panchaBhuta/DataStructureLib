...
//  unittest::ExpectEqual(bool, insertResultExpected, insertResult);
  ...
  std::vector<decltype(vob)::t_versionDate> startDates{};  // in pure forward build, 'startDates' is not used, hence we pass it as empty vector.

  auto buildResult = vob.buildBiDirectionalTimeline( startDates, vo);
  unittest::ExpectEqual(decltype(vob)::t_deltaEntriesMap_iter_diff_type, 3, buildResult.first); // 3 calls to combo-insertDeltaVersion()
  ...
  unittest::ExpectEqual(decltype(vob)::t_deltaEntriesMap_iter_diff_type, 1, buildResult.first); // 1 calls to combo-insertDeltaVersion()
  unittest::ExpectEqual(decltype(vob)::t_deltaEntriesMap_iter_diff_type, 0, buildResult.second);
