                        ...
                        [[maybe_unused]] const t_versionDate&  listingDate)
{
  ...
  unittest::ExpectEqual(typename t_versionObjectBuilder::t_deltaEntriesMap_iter_diff_type, 3, buildResult.first); // 3 calls to insertDeltaVersion()/vob.insertSnapshotVersion()
  ...
  {
    // on second run
    voStrBidi += "07-Apr-2021," TEST_WITH_METADATA("+|@marketLotSpot,") "ANDHRAPAP,ANDHRA PAPER LIMITED,EQ,10,2,INE435A01028,10,LISTED\n";          // FORWARD
  }

  unittest::ExpectEqual(std::string, voStrBidi, t_versionObjectStream::createVOstreamer(vo).toCSV(TEST_WITH_METADATA(dsvo::MetaDataSource::delimiter)));
  ...
  unittest::ExpectEqual(typename t_versionObjectBuilder::t_deltaEntriesMap_iter_diff_type, 1, buildResult.first); // 1 calls to insertDeltaVersion()
  unittest::ExpectEqual(typename t_versionObjectBuilder::t_deltaEntriesMap_iter_diff_type, 0, buildResult.second);
