...
// compare with testBuildBidirectionTimelineNoMetaData_multiReverse_v1.cpp
...
{
  {
    ...
    t_changesInDataSet lotChange {lotChangeFlg, lotChgOldInfo, lotChgNewInfo,
                                  TEST_ALTERNATE_METADATA(lotSizeChgMeta, t_eDataBuild::REVERSE)};   // Delta Change
...

  {
    ...
    VERSIONEDOBJECT_DEBUG_MSG( "debug_LOG: vo.insertVersion() -> afterRelisted-next-versionDate: " << afterRelistedVersionDate << "; DATASET{" << companyRecordAfterRelistedExpected.toCSV(TEST_WITH_METADATA(dsvo::MetaDataSource::delimiter)) << "}");
  ...
  unittest::ExpectEqual(std::string, voStrBidirection, t_versionObjectStream::createVOstreamer(vo).toCSV(TEST_WITH_METADATA(dsvo::MetaDataSource::delimiter)));
}
